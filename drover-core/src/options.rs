...
/// Default interval (seconds) for the voice-bypass keepalive. Non-zero so a fresh
/// install keeps the DPI fooled on long calls; set `udp-keepalive = 0` to disable.
pub const DEFAULT_UDP_KEEPALIVE_SECS: u64 = 15;

#[derive(Debug, Clone)]
pub struct DroverOptions {
    pub proxy: String,
    pub udp_bypass: UdpBypassMode,
    /// How often (seconds) to re-send the UDP fake packet on an active voice socket.
    /// `0` disables the keepalive and restores the send-once behaviour.
    pub udp_keepalive_secs: u64,
}
        ...
        Ok(ini) => {
            ...
            DroverOptions {
                ...
                udp_bypass: section
                    ...
                    .unwrap_or_default(),
                udp_keepalive_secs: section
                    .and_then(|s| s.get("udp-keepalive"))
                    .and_then(|value| value.trim().parse().ok())
                    .unwrap_or(DEFAULT_UDP_KEEPALIVE_SECS),
            }
        }
...
pub fn save_options(path: impl AsRef<Path>, options: &DroverOptions) -> std::io::Result<()> {
    ...
    ini.with_section(Some("drover"))
        ...
        .set("udp-keepalive", options.udp_keepalive_secs.to_string());
